...
data_dir = 'E:\\Download\\fruits-360_dataset\\fruits-360\\' 
...
def test_inputs(filename, batch_size):
    image, label = read_image_data(filename)
    image = adjust_image_for_test(image)
...
weights = {
    'conv_weight1': _variable_with_weight_decay('conv_weight1', [5, 5, 4, 16], tf.truncated_normal_initializer(stddev=5e-2, dtype=tf.float32)),
    ...
    for i in range(1, iterations + 1):
        with tf.Graph().as_default():
            ...
            if i % display_interval == 0 or i == 1:
                loss, acc = sess.run([loss_operation, accuracy], feed_dict={X: batch_x, Y: batch_y, keep_prob: 1})
...
def test_model():
    ...
    while images_left_to_process > 0:
        ...
        results = sess.run(correct_prediction, feed_dict={X: batch_x, Y: batch_y, keep_prob: 1})
        images_left_to_process = images_left_to_process - batch_size
        correct = correct + np.sum(results)
        print("Predicted %d out of %d; partial accuracy %.4f" % (correct, test_images_count - images_left_to_process, correct / (test_images_count - images_left_to_process)))
